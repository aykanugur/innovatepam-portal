...
## V2.0 Terms (Multi-Stage Review Pipeline)

_Added in v1.1 — 2026-02-27_

---

### Review Pipeline

**Definition:** A named, per-category sequence of review stages configured by SUPERADMIN. Each idea category is served by exactly one active pipeline. A pipeline determines how many stages an idea must pass through before a final decision is reached.

**Model:** `ReviewPipeline` in Prisma schema.  
**Key fields:** `name`, `categorySlug`, `blindReview: Boolean`.

---

### Pipeline Stage

**Definition:** A single step within a `ReviewPipeline`. Stages are ordered by `order: Int`. One stage may be designated `isDecisionStage: true` — completing that stage with PASS triggers `ACCEPTED`; with REJECT triggers `REJECTED`.

**Model:** `PipelineStage` in Prisma schema.

---

### IdeaStageProgress

**Definition:** A per-idea, per-stage record that tracks reviewer assignment and outcome. Created when an idea enters a pipeline. One record exists per idea/stage combination.

**Model:** `IdeaStageProgress` in Prisma schema.  
**Key fields:** `ideaId`, `stageId`, `reviewerId` (nullable — assigned on claim), `outcome: StageOutcome`, `completedAt`.

---

### Stage Outcome

**Definition:** The result of a stage evaluation. One of:

| Outcome       | Meaning                                                             |
| ------------- | ------------------------------------------------------------------- |
| `PENDING`     | Not yet claimed                                                     |
| `IN_PROGRESS` | Claimed, under review                                               |
| `PASS`        | Stage passed; advance to next stage (or ACCEPTED if decision stage) |
| `REJECT`      | Stage failed; idea → REJECTED immediately                           |
| `ESCALATE`    | ADMIN cannot decide; escalated to SUPERADMIN for resolution         |

---

### Escalation

**Definition:** A workflow event where an ADMIN, unable to reach a decision on a stage, escalates it to SUPERADMIN. The SUPERADMIN then resolves the escalation with PASS or REJECT via `resolveEscalation()`.

**Trigger:** ADMIN selects the `ESCALATE` outcome when completing a stage.  
**Resolution:** SUPERADMIN calls `lib/actions/resolve-escalation.ts`.

---

### Blind Review

**Definition:** A pipeline-level confidentiality setting that hides the idea author's identity from ADMINs while the idea is `UNDER_REVIEW`. When enabled (`pipeline.blindReview = true` AND `FEATURE_BLIND_REVIEW_ENABLED = true`), ADMINs see `'Anonymous'` instead of the submitter's display name.

**Implementation:** `lib/blind-review.ts` — `maskAuthorIfBlind()`.  
**Scope:** Only applies during `UNDER_REVIEW` status, only to the `ADMIN` role (SUPERADMIN always sees real name; submitter always sees own name).

---

### Smart Form Fields

**Definition:** Dynamic, category-specific form fields that appear on the idea submission form based on the selected category. Defined in `constants/field-templates.ts` as `FieldTemplate[]`.

**Types:** `text`, `textarea`, `select`, `number`, `boolean`  
**Storage:** `Idea.dynamicFields: Json` in Prisma.

---

### Self-Review Guard (V2)

**Rule (extended from Phase 1):** No admin — including SUPERADMIN — may claim or complete a stage review for an idea they authored.  
**Enforcement:** `claim-stage.ts` and `complete-stage.ts` check `idea.authorId !== session.user.id`. Returns `{ code: 'FORBIDDEN' }`.  
**Also applies to:** Escalation resolution — SUPERADMIN cannot resolve an escalation on their own idea.
