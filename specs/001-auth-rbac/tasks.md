...
- [x] T027 [P] Add `"db:seed": "tsx prisma/seed.ts"` to `package.json` scripts and `"test:unit": "vitest run __tests__/unit"` (verify T002 was applied correctly; update if tsx not available — use `ts-node --esm` or `npx tsx`)
- [x] T028 [P] Add Auth.js v5 error code mapping in `auth.ts` to surface `UnverifiedEmail` custom error code: throw `CredentialsSignin` with custom `code` field when `emailVerified === false` so `login-form.tsx` can show the specific message
- [x] T029 Perform full quality gate pass: `npm run type-check` (0 errors) → `npm run lint` (0 warnings) → `npm run test` (≥80% coverage) → `npm run build` (clean build); fix any issues found
